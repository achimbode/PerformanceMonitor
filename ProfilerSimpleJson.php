...
class ProfilerSimpleJson extends ProfilerSimple {
	...
	function logData() {
		...
		// Simon: Ich versuche das Ganze in Files, die mit Timestamps versehen werden, zu schreiben
		// Achim: Simon, du bist ein Held!!! Es funktioniert!
		global $wgTitle;
		$localServer = LocalServer::getImplementation();
		
		$this->collectFileData( $localServer );         // page name, file type, ...
		$filename = $this->assembleFilename( $localServer );   // create filename from timestamp and page title
	...
	protected function assembleFilename( $localServer ) {
		
		$logfolder = $localServer->getLogfilePath();
